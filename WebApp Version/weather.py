...
from botEngine import threading
...
def get_nlp(notify_user=None):
    global _nlp
    if _nlp is None:
        if notify_user:
            print("notifing user")
            notify_user("Getting data...")
        _nlp = stanza.Pipeline(lang='en', processors='tokenize,ner', verbose=False)
    return _nlp

def ner_location_tokens(text, notify_user=None):
    nlp = get_nlp(notify_user)
    doc = nlp(text)
    tokens = []
    covered_words = set()
    added = False
...
def extract_location(user_input, notify_user=None):
    text = user_input.replace("?", "").replace(".", "").strip()
    words = text.lower().split()

    ner_tokens, covered = ner_location_tokens(text, notify_user)
...
def get_coordinates(user_input, notify_user=None):
    city, state, country = extract_location(user_input, notify_user=notify_user)
...
def get_weather(user_input, notify_user=None):
    coords = get_coordinates(user_input, notify_user)
    ...
    # If state is same as city, ignore it
    if state and state.lower() == city_name.lower():
        state = ""
